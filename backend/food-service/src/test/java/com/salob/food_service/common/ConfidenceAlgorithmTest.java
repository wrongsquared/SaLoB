...
class ConfidenceAlgorithmTest {
	...
	/*
	 * Why @Test on an instance method and not static? JUnit 5 creates a NEW
	 * instance of this test class for EACH @Test method. This is intentional — each
	 * test runs in isolation, no shared state. We could make the algorithm a field
	 * (new up once per test), or create it inside each test. Both work. We do it
	 * inline for clarity here.
	 */
	...
	void convertWtfScoreToMultiplier_returns0point1_atScore0() {
		...
		/*
		 * assertEquals(expected, actual, delta) delta = 0.0001 means
		 * "allow this much floating-point error". Floating-point math is never exact
		 * (0.1 might be 0.099999...), so we always use a tolerance with double
		 * assertions.
		 */
	...
	void convertWtfScoreToMultiplier_clampsNegativeScoresTo0() {
		ConfidenceAlgorithm algo = new ConfidenceAlgorithm();
		/*
		 * The function first clamps input to [0, 100], so -50 becomes 0. We expect the
		 * multiplier for 0.
		 */
		double result = algo.convertWtfScoreToMultiplier(-50);
		assertEquals(0.1, result, 1e-9);
	...
	void convertWtfScoreToMultiplier_isMonotonicIncreasing() {
		/*
		 * Property-based test: as WTF score increases, the multiplier should ALWAYS
		 * increase (never decrease). This is a stronger guarantee than testing
		 * individual points.
		 */
		ConfidenceAlgorithm algo = new ConfidenceAlgorithm();
		...
		for (int score = 1; score <= 100; score++) {
			...
			assertTrue(current >= prev, "Multiplier should not decrease at WTF=" + score);
			prev = current;
		}
	}
	...
	void applyConcaveTimeDecay_preservesFullConfidence_atZeroYears() {
		ConfidenceAlgorithm algo = new ConfidenceAlgorithm();
		/*
		 * If foodEntryAgeInYears = 0: nonDecayPercent = -0^2 + 100 = 100 result = 80 *
		 * (100/100) = 80
		 */
	...
	void applyConcaveTimeDecay_reducesToZero_atTenYears() {
		ConfidenceAlgorithm algo = new ConfidenceAlgorithm();
		/*
		 * If foodEntryAgeInYears = 10: nonDecayPercent = -100 + 100 = 0 result = 80 *
		 * (0/100) = 0
		 */
	...
	void applyConcaveTimeDecay_neverReturnsNegative() {
		/*
		 * Even with a very negative pre-decay confidence (which shouldn't happen in
		 * practice since we clamp), the result should be >= 0.
		 */
		ConfidenceAlgorithm algo = new ConfidenceAlgorithm();
	...
	void computeFinalConfidence_returnsZero_forEntryWithNoVotes() {
		/*
		 * FoodEntry has @Builder from Lombok. We use the builder pattern to create an
		 * entry with ZERO votes and a known creation time.
		 *
		 * builder().field(value).build() — only set what matters, defaults for rest.
		 */
		FoodEntry entry = FoodEntry.builder().votes(List.of()) // explicitly empty
				.build();
		/*
		 * createdAt and id are inherited from BaseEntity (via @Getter @Setter).
		 * Lombok's @Builder on a subclass does NOT generate builder methods for
		 * inherited fields — we must set them via setters after building.
		 */
		...
		/*
		 * With no votes: rawConfidence stays at 0. At 0 years old: nonDecayPercent =
		 * 100, so result = 0 * (100/100) = 0.
		 */
		assertEquals(0.0, result, 1e-9);
	}
	...
	void computeFinalConfidence_clampsTo100_forExtremelyHighConfidence() {
		/*
		 * This test creates a very "old" entry (10 years) where decay = 0%. Since we
		 * pass zero votes, the final result should still be 0.
		 *
		 * We can't easily test the full path with votes here because the gRPC stub is
		 * null. That test belongs in EateryServiceTest where we can mock the gRPC
		 * client properly.
		 *
		 * This test verifies the clamp at [0, 100] works.
		 */
		FoodEntry entry = FoodEntry.builder().votes(List.of()).build();
		entry.setCreatedAt(Instant.now());
