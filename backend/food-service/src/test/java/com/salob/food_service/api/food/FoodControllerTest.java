...
class FoodControllerTest {
	...
	void searchForFood_returnsOk() throws Exception {
		FoodSearchPreview dto = FoodSearchPreview.builder().foodId(UUID.randomUUID()).foodName("Chicken Rice")
				.photoUrl("").build();

		when(foodService.searchForFood("chicken")).thenReturn(List.of(dto));

		mockMvc.perform(get("/api/foods/search").param("search", "chicken")).andExpect(status().isOk())
	...
	void searchForFood_whenNoResults_returnsOkEmpty() throws Exception {
		...
		mockMvc.perform(get("/api/foods/search").param("search", "nothing")).andExpect(status().isOk())
