...
class FoodEntryControllerTest {
	...
	@BeforeEach
	void setUp() {
		FoodEntryController controller = new FoodEntryController(foodEntryService);
	...
	void getHistoricalData_returnsOk() throws Exception {
		...
		when(foodEntryService.getFoodEntryHistoricalData(eq(entryId), any(Instant.class), isNull())).thenReturn(dto);
