...
public class FoodService {
	...
	public List<FoodSearchPreview> searchForFood(String search) {
		return foodRepo.findByLabelContainingIgnoreCase(search).stream()
				.map(f -> FoodSearchPreview.builder().foodId(f.getId()).foodName(f.getLabel()).photoUrl("").build())
	 ...
	 * check (e.g., "Is 'Dragon Phoenix Rice' a real dish?"). Flow: 1. Check
	 * cache/DB for existing food (current behavior) 2. If new, send to AI
	 * verification service 3. AI returns confidence score; if below threshold, flag
	 * for human review 4. Frontend receives status: "pending_verification" or
	 * "approved"
	 */
	...
	public FoodSearchPreview createFood(FoodCreationRequest req) {
		...
		for (Food f : existing) {
			if (f.getLabel().equalsIgnoreCase(trimmedName)) {
				...
				return FoodSearchPreview.builder().foodId(f.getId()).foodName(f.getLabel()).photoUrl("").build();
			}
		}

		// Create new food
		Food newFood = Food.builder().label(trimmedName).photoObjKey("").build();
		Food saved = foodRepo.save(newFood);
		log.info("Created new food: {} (id={})", trimmedName, saved.getId());

		return FoodSearchPreview.builder().foodId(saved.getId()).foodName(saved.getLabel()).photoUrl("").build();
	}
}
