...
public class EaterySeeder {
	...
	public List<Eatery> seed(List<EateryType> eateryTypes) {
		Map<String, EateryType> typesByLabel = eateryTypes.stream()
		...
		List<EaterySeedSpec> eateries = List.of(
				...
				new EaterySeedSpec("Ya Kun Kaya Toast (Far East Square)", "7 Chatterton Road", "Kopitiam", 1.2835,
						103.8483),
				new EaterySeedSpec("The Daily Cut (Tanjong Pagar)", "15 Tanjong Pagar Road", "Bistro", 1.2778,
						103.8463),
				...
				// --- EAST (Foodie Haven) ---
				new EaterySeedSpec("Old Airport Road Food Centre", "51 Old Airport Road", "Hawker Stall", 1.3083,
						103.8858),
				new EaterySeedSpec("East Coast Lagoon Food Village", "1220 East Coast Parkway", "Hawker Stall", 1.3075,
						103.9351),
				...
				new EaterySeedSpec("Birds of Paradise Gelato", "H28, 63 Aung San Avenue", "Dessert Shop", 1.3038,
						103.9065),
				new EaterySeedSpec("Changi Village Hawker Centre", "2 Changi Village Road", "Hawker Stall", 1.3891,
						103.9882),
				new EaterySeedSpec("Bedok 85 (Fengshan Market)", "85 Bedok North Street 4", "Hawker Stall", 1.3321,
						103.9382),
				...
				new EaterySeedSpec("Creamier Handcrafted Ice Cream", "5 Clementi Road", "Dessert Shop", 1.2801,
						103.8033),
				...
				// --- NORTH / NORTH-EAST ---
				new EaterySeedSpec("Chomp Chomp Food Centre", "20 Kensington Park Road", "Hawker Stall", 1.3551,
						103.8665),
				new EaterySeedSpec("Serangoon Garden Market", "49A Serangoon Garden Way", "Hawker Stall", 1.3555,
						103.8667),
				...
				new EaterySeedSpec("Punggol Settlement Seafood", "2 Punggol Point Road", "Restaurant", 1.4069,
						103.9238),
				new EaterySeedSpec("Whampoa Drive Food Centre", "2 Whampoa Drive", "Hawker Stall", 1.3232, 103.8546),
				new EaterySeedSpec("Upper Thomson Prata House", "32 Upper Thomson Road", "Restaurant", 1.3524,
						103.8341),
				...
				new EaterySeedSpec("Toa Payoh Lucky Pisang Raja", "4 Toa Payoh Central", "Dessert Shop", 1.3322,
						103.8475),
				...
				new EaterySeedSpec("The Whale Tea (Lot One)", "2 Choa Chu Kang Avenue 4", "Bubble Tea Shop", 1.3852,
						103.7475),
				...
				new EaterySeedSpec("PastaMania (Clementi Mall)", "3155 Commonwealth Avenue", "Fast Food", 1.3150,
						103.7641),
				new EaterySeedSpec("Saizeriya (Liang Court)", "177 River Valley Road", "Restaurant", 1.2913, 103.8445),
				new EaterySeedSpec("Marche Movenpick", "3A Level 1, 9 Raffles Boulevard", "Restaurant", 1.3011,
						103.8394),
	...
	private void seedEateryIfMissing(EaterySeedSpec spec, Map<String, EateryType> typesByLabel, Random random) {
		...
		if (!minioStorageService.objectExists(objectKey)) {
			if (!seedImageHelper.isImageOnDisk(diskPath)) {
				throw new IllegalStateException("Missing eatery seed image at " + diskPath
			...
			if (uploadedKey == null) {
				throw new IllegalStateException("Failed to upload eatery seed image to MinIO: " + diskPath);
			}
	...
	private void saveEatery(EaterySeedSpec spec, EateryType type, Random random, String objectKey) {
		...
		eateryRepo.save(Eatery.builder().name(spec.name()).type(type).address(spec.address()).isOpen(isOpen)
				.photoObjKey(objectKey).location(geometryFactory.createPoint(new Coordinate(spec.lon, spec.lat)))
				.build());
	}
}
