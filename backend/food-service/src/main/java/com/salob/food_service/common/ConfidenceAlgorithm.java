...
/**
 * const finalConfidenceScore = foodEntry => { // Refer to NOTE below regarding
 * the 'votesRequiredForMaxScore' const baseVoteValue = 100 /
 * votesRequiredForMaxScore; let conf = 0;
 *
 * for (const vote of foodEntry.votes) { const actualVoteValue = vote.isUpvote ?
 * baseVoteValue : -baseVoteValue; conf += actualVoteValue *
 * computeMultiplier(vote.voter.WTFScore); // Lagrange }
 *
 * const decayedConf = decay(foodEntry.ageInYears); return clamp(0, 100,
 * decayedConf); };
 */
@Component
public class ConfidenceAlgorithm {
	...
	/**
	 * NOTE: 'votesRequiredForMaxScore' is the number of upvotes that are required
	 * from people (each with a WTF score of 50) on a food entry (with no
	 * downvotes!) to reach a confidence score of 100 (not taking into account time
	 * decay).
	 */
	...
	private Map<UUID, Double> batchFetchWtfScores(List<FoodEntryVote> votes) {
		List<String> voterIds = votes.stream().map(v -> v.getVoterId().toString()).distinct().toList();

		if (voterIds.isEmpty()) {
			return new HashMap<>();
		}

		UserWtfBatchRequest req = UserWtfBatchRequest.newBuilder().addAllUserIds(voterIds).build();
		...
		try {
			...
			log.warn(
					"gRPC batch WTF fetch from user-service failed — falling back to default scores (50.0) for all voters",
					e);
		}
		return wtfMap;
	}
	...
	/**
	 * Package-private for testing purposes. Converts a WTF score [0, 100] to a
	 * multiplier [0.1, 2.5] using Lagrange interpolation.
	 */
	...
	/**
	 * Package-private for testing purposes. Applies concave time decay to
	 * confidence scores based on age in years.
	 */
