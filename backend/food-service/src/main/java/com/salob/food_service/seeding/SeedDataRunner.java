 ...
 *
 * Seeding order matters: 1. EateryTypes (no dependencies) 2. Eateries (depends
 * on types) 3. Foods (no dependencies) 4. FoodEntries (depends on foods +
 * eateries) 5. FoodEntryVotes (depends on food entries + user IDs)
 *
 * Reset behavior: - If app.seed.reset=true, deletes all data before reseeding
 * (fast for stress tests) - Otherwise, idempotent seeding (safe to run multiple
 * times)
 */
@Slf4j
@Component
...
public class SeedDataRunner implements CommandLineRunner {
	...
	/**
	 * User-Service MUST be running, we're making a gRPC call there to fetch all the
	 * user IDs
	 */
	private List<UUID> retrieveAllIDsFromUserService() {
		try {
			...
			return allUserIDs.getIdsList().stream().map(uuidStr -> {
				try {
