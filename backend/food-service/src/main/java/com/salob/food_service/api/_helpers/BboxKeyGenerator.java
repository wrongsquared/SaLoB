 ...
 *
 * @Cacheable(key = "@bboxCacheKeyGenerator.generate(#minLat, ...)") └─ Fragile:
 *                string-based, breaks if method names change, no IDE support
 *
 *                We use Spring's KeyGenerator interface:
 * @Cacheable(keyGenerator = "bboxKeyGenerator") └─ Type-safe: Java method
 *                         references, IDE support, refactor-friendly
 *
 *                         This is the "official" Spring way to do custom key
 *                         generation.
 */
...
public class BboxKeyGenerator implements KeyGenerator {
	 ...
	 *
	 * Spring calls this automatically when @Cacheable is used with this
	 * KeyGenerator.
	 *
	 * @param target
	 *            the object instance (we don't use it here)
	 * @param method
	 *            the method being called (we don't use it here)
	 * @param params
	 *            the method parameters: [minLat, maxLat, minLon, maxLon]
	...
	public Object generate(Object target, Method method, Object... params) {
		if (params.length != 4) {
			throw new IllegalArgumentException(
					"BboxKeyGenerator expects exactly 4 parameters (minLat, maxLat, minLon, maxLon), got "
							+ params.length);
		}
	...
	private String generateBucketedKey(double minLat, double maxLat, double minLon, double maxLon) {
		...
		return String.format("bbox:%.2f:%.2f:%.2f:%.2f", bucketMinLat, bucketMaxLat, bucketMinLon, bucketMaxLon);
	}
}
