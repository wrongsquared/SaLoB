...
public interface EateryRepository extends JpaRepository<Eatery, UUID> {
	 ...
	 *
	 * This uses a native SQL query with PostGIS functions: - ST_MakeEnvelope:
	 * Creates a rectangle from (minLon, minLat) to (maxLon, maxLat) - ST_Contains:
	 * Checks if the eatery's point location is inside the rectangle - SRID 4326:
	 * WGS84 coordinate system (standard GPS)
	 *
	 * Why native query? JPQL doesn't have good geometry support, so we use raw SQL
	 * with PostGIS functions that the database engine optimizes with spatial
	 * indices.
	 *
	 * @param minLat
	 *            southern boundary (example: 1.27)
	 * @param maxLat
	 *            northern boundary (example: 1.32)
	 * @param minLon
	 *            western boundary (example: 103.80)
	 * @param maxLon
	 *            eastern boundary (example: 103.86)
