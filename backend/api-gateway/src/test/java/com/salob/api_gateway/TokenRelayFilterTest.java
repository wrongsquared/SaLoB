...
class TokenRelayFilterTest {
	...
	void filter_withFullClaims_addsHeaders() {
		Jwt jwt = Jwt.withTokenValue("token").header("alg", "RS256").subject("user-abc-123")
				.claim("username", "johndoe").claim("roles", List.of("CONTRIBUTOR")).build();
		...
		StepVerifier.create(
				filter.filter(exchange, chain).contextWrite(ReactiveSecurityContextHolder.withAuthentication(auth)))
				.verifyComplete();
	...
	void filter_withoutRolesClaim_setsEmptyRolesHeader() {
		Jwt jwt = Jwt.withTokenValue("token").header("alg", "RS256").subject("user-xyz").claim("username", "janedoe")
				.build();
		Authentication auth = new JwtAuthenticationToken(jwt);
	...
	void filter_withNullSubjectAndUsername_skipsThoseHeaders() {
		Jwt jwt = Jwt.withTokenValue("token").header("alg", "RS256").claim("roles", List.of("ADMIN")).build();
		Authentication auth = new JwtAuthenticationToken(jwt);
	...
	void filter_withMultipleRoles_joinsWithComma() {
		Jwt jwt = Jwt.withTokenValue("token").header("alg", "RS256").subject("user-456").claim("username", "bob")
				.claim("roles", List.of("ADMIN", "CONTRIBUTOR", "USER")).build();
		Authentication auth = new JwtAuthenticationToken(jwt);
