...
/**
 * Consumes WTF events from RabbitMQ, checks idempotency,
 * and delegates to {@link WtfRecalculationService}.
 */
@Slf4j
@Component
...
public class WtfEventConsumer {
    ...
    public void handleWtfEvent(WtfEvent event) {
        ...
        if (processedEventRepo.existsById(event.eventId())) {
            log.info("Skipping duplicate WTF event: eventId={}, type={}",
                    event.eventId(), event.type());
            return;
        }

        log.info("Processing WTF event: type={}, eventId={}, targetUserId={}",
                event.type(), event.eventId(), event.targetUserId());
        ...
        try {
            ...
            log.error("Failed to process WTF event: eventId={}, type={}",
