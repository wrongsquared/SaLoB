...
public class AuthService {
	...
	public void register(RegisterRequest req) {
		...
		var user = User.builder().username(req.username()).email(req.email())
				.passwordHash(passwordEncoder.encode(req.password())).roles(roles).authProvider(AuthProvider.LOCAL)
				.build();
		userRepo.save(user);
	}
}
