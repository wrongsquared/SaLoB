...
public class UserGrpcHandler extends UserServiceGrpc.UserServiceImplBase {
	...
	@Override
	public void getUserWtfScoreBatch(UserWtfBatchRequest request,
			StreamObserver<UserWtfBatchResponse> responseObserver) {
		try {
			List<UUID> userIds = request.getUserIdsList().stream().map(UUID::fromString).toList();

			List<UserWtfBatchResponseItem> wtfBatchResponseItems = userService.getUserWtfScoreBatch(userIds).stream()
					.map(item -> UserWtfBatchResponseItem.newBuilder().setUserId(item.userId().toString())
							.setWtfScore(item.wtfScore()).build())
					.toList();
	...
	public void getUserDetails(UserDetailsRequest request, StreamObserver<UserDetailsResponse> responseObserver) {
		try {
			...
			UserDetailsResponse response = UserDetailsResponse.newBuilder().setUserId(request.getUserId())
