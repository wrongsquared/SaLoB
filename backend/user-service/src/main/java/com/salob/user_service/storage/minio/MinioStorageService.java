...
public class MinioStorageService {
	...
	public boolean objectExists(String objectKey) {
		createBucketIfNotExists(properties.getBucket());
		try {
			minioClient.statObject(StatObjectArgs.builder().bucket(properties.getBucket()).object(objectKey).build());
			return true;
		} catch (Exception e) {
			return false;
		}
	...
	public String getPresignedUrl(String objectKey, Duration expiry) {
		...
		createBucketIfNotExists(properties.getBucket());
		try {
			return minioClient.getPresignedObjectUrl(GetPresignedObjectUrlArgs.builder().method(Http.Method.GET)
					.bucket(properties.getBucket()).object(objectKey).expiry((int) expiry.toSeconds()).build());
		} catch (Exception e) {
