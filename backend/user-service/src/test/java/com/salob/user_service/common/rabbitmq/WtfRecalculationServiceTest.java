...
class WtfRecalculationServiceTest {
	...
	void newUserGetsBaseline50() {
		User user = User.builder().id(UUID.randomUUID()).createdAt(Instant.now()).totalSubmissions(0).upvotesReceived(0)
				.downvotesReceived(0).anomaliesFlagged(0).build();
	...
	void activeTrustedUserGetsHighScore() {
		// User with 1 year tenure, 20 submissions, 10 upvotes, 0 downvotes, active
		// today
		User user = User.builder().id(UUID.randomUUID()).createdAt(Instant.now().minus(365, ChronoUnit.DAYS))
				.lastActivityAt(Instant.now()).totalSubmissions(20).upvotesReceived(10).downvotesReceived(0)
				.anomaliesFlagged(0).build();
	...
	void inactiveUserGetsHalvedScore() {
		User user = User.builder().id(UUID.randomUUID()).createdAt(Instant.now().minus(365, ChronoUnit.DAYS))
				.lastActivityAt(Instant.now().minus(200, ChronoUnit.DAYS)).totalSubmissions(20).upvotesReceived(10)
				.downvotesReceived(0).anomaliesFlagged(0).build();
	...
	void fullyFlaggedUserGetsZeroFlagScore() {
		User user = User.builder().id(UUID.randomUUID()).createdAt(Instant.now()).lastActivityAt(Instant.now())
				.totalSubmissions(5).upvotesReceived(0).downvotesReceived(0).anomaliesFlagged(5).build();
	...
	void heavilyDownvotedUserScoresLow() {
		User user = User.builder().id(UUID.randomUUID()).createdAt(Instant.now()).lastActivityAt(Instant.now())
				.totalSubmissions(10).upvotesReceived(0).downvotesReceived(20).anomaliesFlagged(0).build();
