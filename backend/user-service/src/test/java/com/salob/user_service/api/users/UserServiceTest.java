...
class UserServiceTest {
	...
	void me_returnsUserProfile() {
		...
		User user = User.builder().email("test@example.com").username("testuser")
				.authProvider(com.salob.user_service.api.auth.AuthProvider.LOCAL).avatarObjKey("avatar-key").build();
		user.setId(userId);

		when(userRepo.findById(userId)).thenReturn(Optional.of(user));
		when(minioService.getPresignedUrl("avatar-key", Duration.ofMinutes(15))).thenReturn("https://avatar.url");
	...
	void me_whenNotFound_throws404() {
		...
		assertThrows(ResponseStatusException.class, () -> userService.me(UUID.randomUUID()));
	}

	@Test
	void getUserWtfScore_returnsScore() {
		UUID userId = UUID.randomUUID();
		User user = User.builder().wtfScore(65.0).authProvider(com.salob.user_service.api.auth.AuthProvider.LOCAL)
				.build();
		user.setId(userId);
	...
	void getUserWtfScoreBatch_returnsScoresForFoundUsers() {
		...
		User user1 = User.builder().wtfScore(70.0).authProvider(com.salob.user_service.api.auth.AuthProvider.LOCAL)
				.build();
		user1.setId(id1);
