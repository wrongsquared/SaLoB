...
const EATERIES_MAP = [
  {
    ...
    latitude: 1.2835,
    longitude: 103.8437,
    typeLabel: 'Hawker Stall',
  },
  {
    eateryId: 'a1b2c3d4-e29b-41d4-a716-446655440002',
  ...
  {
    eateryId: 'a1b2c3d4-e29b-41d4-a716-446655440003',
  ...
  {
    eateryId: 'a1b2c3d4-e29b-41d4-a716-446655440004',
    name: 'Newton Food Centre',
    latitude: 1.3115,
    longitude: 103.8375,
    typeLabel: 'Hawker Centre',
  },
  {
    eateryId: 'a1b2c3d4-e29b-41d4-a716-446655440005',
...
const FOOD_PREVIEWS = [
  {
    foodEntryId: 'b2c3d4e5-e29b-41d4-a716-446655440001',
    name: 'Chicken Rice',
    sgCents: 450,
    upvotes: 23,
    downvotes: 2,
    photoPresignedUrl: '/mock/chicken-rice.jpg',
    submitterId: MOCK_USER_ID,
    submitterUsername: 'foodie_alan',
    createdAt: '2026-05-01T10:00:00Z',
  },
  {
    foodEntryId: 'b2c3d4e5-e29b-41d4-a716-446655440002',
    name: 'Laksa',
    sgCents: 680,
    upvotes: 18,
    downvotes: 1,
    photoPresignedUrl: '/mock/laksa.jpg',
    submitterId: MOCK_USER_ID,
    submitterUsername: 'spice_hunter',
    createdAt: '2026-04-28T12:30:00Z',
  },
  {
    foodEntryId: 'b2c3d4e5-e29b-41d4-a716-446655440003',
    name: 'Kaya Toast Set',
    sgCents: 320,
    upvotes: 45,
    downvotes: 3,
    photoPresignedUrl: '/mock/kaya-toast.jpg',
    submitterId: MOCK_USER_ID,
    submitterUsername: 'breakfast_club',
    createdAt: '2026-05-15T08:00:00Z',
  },
  {
    foodEntryId: 'b2c3d4e5-e29b-41d4-a716-446655440004',
    name: 'Satay (10 sticks)',
    sgCents: 800,
    upvotes: 31,
    downvotes: 4,
    photoPresignedUrl: '/mock/satay.jpg',
    submitterId: MOCK_USER_ID,
    submitterUsername: 'grill_master',
    createdAt: '2026-05-10T19:00:00Z',
  },
  {
    foodEntryId: 'b2c3d4e5-e29b-41d4-a716-446655440005',
    name: 'Fishball Noodles',
    sgCents: 380,
    upvotes: 12,
    downvotes: 0,
    photoPresignedUrl: '/mock/fishball-noodles.jpg',
    submitterId: MOCK_USER_ID,
    submitterUsername: 'noodle_whisperer',
...
const FOOD_DB = [
  ...
  {
    foodId: 'c3d4e5f6-e29b-41d4-a716-446655440005',
    foodName: 'Fishball Noodles',
    photoUrl: '/mock/fishball-noodles.jpg',
  },
];
...
export const handlers = [
  ...
  http.get('/api/eateries/search', async ({ request }) => {
    ...
    const results = EATERIES_MAP.filter((e) => e.name.toLowerCase().includes(search)).map((e) => ({
      eateryId: e.eateryId,
      name: e.name,
  ...
  http.get('/api/foods/search', async ({ request }) => {
    ...
    const url = new URL(request.url);
    const search = url.searchParams.get('search')?.toLowerCase() ?? '';
    const results = FOOD_PREVIEWS.filter((f) => f.name.toLowerCase().includes(search)).map((f) => ({
      foodId: f.foodEntryId,
      foodName: f.name,
      photoUrl: f.photoPresignedUrl,
    }));
    return HttpResponse.json(results);
  }),
  ...
  http.get('/api/eateries/search/combined', async ({ request }) => {
    ...
    const url = new URL(request.url);
    const search = url.searchParams.get('search')?.toLowerCase() ?? '';
    const local = EATERIES_MAP.filter((e) => e.name.toLowerCase().includes(search)).map((e) => ({
      eateryId: e.eateryId,
      name: e.name,
      address: '1 Example St, Singapore',
    }));
    const onemap =
      local.length === 0
        ? [
            {
              name: search.charAt(0).toUpperCase() + search.slice(1) + ' (OneMap)',
              address: 'Singapore',
              latitude: 1.3,
              longitude: 103.85,
            },
