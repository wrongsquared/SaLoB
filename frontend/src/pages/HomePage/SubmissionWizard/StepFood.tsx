...
export default function StepFood({ onSelect, onBack }: StepFoodProps) {
  ...
  if (useFreeText) {
    return (
      <div className="space-y-4">
        ...
        <h2 className="text-lg font-semibold text-secondary-900">Enter food name</h2>
        <input
          type="text"
          value={input}
  ...
  return (
    <div className="space-y-4">
      ...
      <h2 className="text-lg font-semibold text-secondary-900">Select a food</h2>
      <input
        type="text"
        value={input}
        onChange={(e) => setInput(e.target.value)}
        ...
        {!isLoading &&
          results?.map((r) => (
            ...
            >
              ...
              <p className="text-sm font-medium text-secondary-900">{r.foodName}</p>
            </button>
          ))}
        {!isLoading && input && results?.length === 0 && (
          <div className="py-4 text-center">
            <p className="text-sm text-secondary-400">No foods found in the database.</p>
            <button
              type="button"
